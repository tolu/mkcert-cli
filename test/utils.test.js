...
import { exec, existsSync, pkgVersion, semverGreaterThan } from "../src/utils.js";
...
describe('cli', () => {
  beforeEach(async () => { await rmTestDir() });
  ...
  it('"--host" sets custom host', async () => {
    const {stdout: output1 } = await exec(`${cmd} --host mkcert-online.com --host mkcert2-online.com -o ${testCertFolder} -d`, {});
